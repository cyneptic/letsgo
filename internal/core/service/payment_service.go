...
func (p *PaymentService) CreateNewPayment(reservationId, payerID string) (string, error) {
	...
	flight, err := p.gormDb.GetFlightById(reservationId)
	if err != nil {
		return "", err
	}
	...
	payload := strings.NewReader(fmt.Sprintf(RequestXMLBody, terminalID, userName, userPassword, reserve.ID, amount, localDate, localTime, additionalData, callBackURL, payerID))
	...
	if code == "0" {
		...
		`, refID)
		p.redisDb.SetPaymentRequest(reservationId, payerID, refID, amount)
...
func (p *PaymentService) VerifyPayment(RefId, orderId, SaleReferenceId string) (bool, error) {
	terminalID := os.Getenv("BANK_TERMINAL_ID")
	userName := os.Getenv("BANK_USERNAME")
	userPassword := os.Getenv("BANK_USER_PASSWORD")

	if ok, err := p.redisDb.VerifyPaymentRequest(RefId, orderId); err != nil || !ok {
		return ok, err
	}
